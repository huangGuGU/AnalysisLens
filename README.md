...
点击右侧 `Lens` 标题可以在镜头使用频率和焦段使用频率之间切换。焦段按 `10-24mm`、`25-40mm`、`41-60mm`、`61-135mm`、`136-200mm`、`201+mm` 的固定顺序展示。

### 使用曲线

点击右侧 `Lens` / `Focal` 标题旁的可视化按钮后，选择某个镜头或焦段会在柱状图上叠加一条平滑的 MA20 使用曲线。按住图表左右拖动时只更新下方日期，不会触发高亮或压暗柱子。

### 缓存管理

从 macOS 顶部菜单栏选择 `Tools` > `Delete Metadata Cache` 可以删除本地 EXIF 元数据缓存。删除缓存不会修改照片文件，下次分析会重新读取照片元数据。
